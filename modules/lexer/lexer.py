...
class LexerService():
	...
	def __test_token(self, input) -> LexerToken:
		token_type = None
		token_value = None
		if self.comment_skip_mode:
	...
	def __test_for_keyword(self, identifier)->LexerToken:
	...
	def __tokenize_string(self, data) -> List[LexerToken]:
		input = data
		tokens = []
		...
		while input:
			...
			if input == "":
				break
			lexerToken: LexerToken = self.__test_token(input)
			match lexerToken.type:
