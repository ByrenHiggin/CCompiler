...
class TokenType(Enum):
    ...
    IFDEF = TokenDefinition(type='IFDEF', pattern=r'\#ifdef\b')
    IFNDEF = TokenDefinition(type='IFNDEF', pattern=r'\#ifndef\b')
    ELSE = TokenDefinition(type='ELSE', pattern=r'\#else\b')
    ENDIF = TokenDefinition(type='ENDIF', pattern=r'\#endif\b')
    PRAGMA = TokenDefinition(type='PRAGMA', pattern=r'\#pragma\b')
    INCLUDE = TokenDefinition(type='INCLUDE', pattern=r'\#include\b')
    IDENTIFIER = TokenDefinition(type='IDENTIFIER', pattern=r'[a-zA-Z_]\w*\b')
    CONSTANT = TokenDefinition(type='CONSTANT', pattern=r'[0-9]+\b')
    STRINGCONSTANT = TokenDefinition(type='STRINGCONSTANT', pattern=r'"(.)*"')
